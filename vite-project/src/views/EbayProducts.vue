...
export default {
  ...
  methods: {
    filterByPrice() {
      axios.get(`https://localhost:8444/second-service/api/v1/ebay/filter/price/${this.priceFrom}/${this.priceTo}?page=${this.page}&size=${this.pageSize}`)
        .then(response => {
    ...
    increasePrices() {
      axios.post(`https://localhost:8444/second-service/api/v1/ebay/price/increase/${this.increasePercent}`)
        .then(() => {
    ...
    deleteProduct(productId) {
      if (confirm('Are you sure you want to delete this product?')) {
        axios.delete(`https://localhost:8090/api/v1/products/${productId}`)
          .then(() => {
          ...
          .catch(error => {
            this.errorMessage = error.response ?
              error.response.data.errors.map(e => e.field ? `${e.field}: ${e.message}` : e.message).join(', ') : 'Server Error';
          });
      }
    }
  }
