...
export default {
  ...
  methods: {
    filterByPrice() {
      axios.get(`https://localhost:8081/second-service/api/v1/ebay/filter/price/${this.priceFrom}/${this.priceTo}?page=${this.page}&size=${this.pageSize}`)
        .then(response => {
    ...
    increasePrices() {
      axios.post(`https://localhost:8081/second-service/api/v1/ebay/price/increase/${this.increasePercent}`)
        .then(() => {
    ...
    deleteProduct(productId) {
      if (confirm('Are you sure you want to delete this product?')) {
        axios.delete(`https://localhost:8081/first-service/api/v1/products/${productId}`)
          .then(() => {
